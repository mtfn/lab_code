...
class Turtlebot4Mekf(Node):
    ...
    def process_covariance(self, time_delta):       #[Checked] [Saimai]
        # Is the From (97) in [4]
        Q = np.zeros(shape=(self.num_states, self.num_states), dtype=float)
        Q[0:3, 0:3] = self.gyro_cov*time_delta + self.gyro_bias_cov*(time_delta**3)/3.0
        Q[0:3, 9:12] = -self.gyro_bias_cov*(time_delta**2)/2.0
        Q[3:6, 3:6] = self.accel_cov*time_delta + self.accel_bias_cov*(time_delta**3)/3.0
        Q[3:6, 6:9] = self.accel_bias_cov * (time_delta**4)/8.0 + self.accel_cov*(time_delta**2)/2.0
        Q[3:6, 12:15] = -self.accel_bias_cov*(time_delta**2)/2.0
        Q[6:9, 3:6] = self.accel_cov * (time_delta**2)/2.0 + self.accel_bias_cov*(time_delta**4)/8.0
        Q[6:9, 6:9] = self.accel_cov * (time_delta**3)/3.0 + self.accel_bias_cov*(time_delta**5)/20.0
    ...
    def imuCallback(self, imu_msg: Imu):
        t = self.get_clock().now()
        self.q.put((t, 'imu', imu_msg.angular_velocity,
                   imu_msg.linear_acceleration))

    def cmdvelCallback(self, cmd_vel_msg: Twist):
        self.isMoving = True 
