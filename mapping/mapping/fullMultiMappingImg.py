...
class fullMulMappingNode(Node):
    def __init__(self):
        ...
        self.getMu = self.create_subscription(CompressedImage, Name_j+"/muCov", self.muCovCallback, 
                                                        QoSProfile(depth=10, reliability=ReliabilityPolicy.BEST_EFFORT))

        self.pubM = self.create_publisher(CompressedImage, Name_i+"/muCov", 10)
    ...
    def muCovCallback(self, msg: CompressedImage):
    ...
    def postMuCov(self):
        ...
        tem4 = np.append(tem4,self.omega_i.flatten())
        # print(np.append(tem2,tem4))
        tem3 = tem2.max()
        tem5 = tem4.max()
        tem2 = tem2/tem3
        tem4 = tem4/tem5
        tem2 = [a*127+127 for a in tem2]
        tem4 = tem4*255
        tem2 = np.append(tem2, tem4)
        tem2 = tem2.astype(np.uint8)
        tem.format = str(tem3)+","+str(tem5)
        tem.data = [int(b) for b in tem2]
